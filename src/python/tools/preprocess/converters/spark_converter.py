...
from pyspark.sql.functions import rand, monotonically_increasing_id, row_number, col
...
def get_nodes_df(edges_df):
    nodes = edges_df.select(col(SRC_COL).alias(NODE_LABEL)).union(edges_df.select(col(DST_COL).alias(NODE_LABEL))) \
        .distinct().repartition(1).orderBy(rand()).cache()
    nodes = assign_ids(nodes, INDEX_COL)
    return nodes


def get_relations_df(edges_df):
    rels = edges_df.drop(SRC_COL, DST_COL).distinct().repartition(1).orderBy(rand()) \
        .withColumnRenamed(REL_COL, RELATION_LABEL).cache()
...
def remap_edges(edges_df, nodes, rels):
    if rels is not None:
        remapped_edges_df = edges_df.join(nodes.hint("merge"), edges_df.src == nodes.node_label) \
...
class SparkEdgeListConverter(object):
    def __init__(self,
        ...
        self.spark = SparkSession.builder.appName(SPARK_APP_NAME) \
        ...
        if format.upper() in SUPPORTED_DELIM_FORMATS:
            self.reader = SparkDelimitedFileReader(self.spark,
                                                   train_edges,
                                                   valid_edges,
                                                   test_edges,
                                                   columns,
        ...
        if train_edges_df is None:
            ...
            if self.test_split is not None:
                ...
                if self.valid_split is not None:
                    ...
                    print("Splitting into: {}/{}/{} fractions".format(self.train_split,
                                                                      self.valid_split,
                                                                      self.test_split))
                    ...
                    train_edges_df, test_edges_df = all_edges_df.randomSplit(
                        [self.train_split, self.test_split])
            else:
                train_edges_df = all_edges_df
        all_edges_df, train_edges_df, valid_edges_df, test_edges_df = assign_ids(all_edges_df, EDGES_INDEX_COL), assign_ids(train_edges_df, EDGES_INDEX_COL), assign_ids(valid_edges_df, EDGES_INDEX_COL), assign_ids(test_edges_df, EDGES_INDEX_COL)        

        if self.partitioner is not None:
            print("Partition nodes into {} partitions".format(self.num_partitions))
            train_edges_df, valid_edges_df, test_edges_df = self.partitioner.partition_edges(train_edges_df,
                                                                                             valid_edges_df,
                                                                                             test_edges_df,
                                                                                             nodes_df,
                                                                                             self.num_partitions)

        return self.writer.write_to_binary(train_edges_df,
                                           valid_edges_df,
                                           test_edges_df,
                                           nodes_df,
                                           rels_df,
