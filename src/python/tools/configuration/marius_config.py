...
class DatasetConfig:
    ...
    def __post_init__(self):
        ...
        dataset_stats_path = Path(self.base_directory) / Path("dataset.yaml")
        if not dataset_stats_path.exists():
            raise ValueError("{} does not exist".format(str(dataset_stats_path)))

        edges_path = Path(self.base_directory) / Path("edges")
        ...
        nodes_path = Path(self.base_directory) / Path("nodes")
