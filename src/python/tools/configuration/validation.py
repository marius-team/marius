...
def validate_dataset_config(output_config):
    ...
    if output_config.model.learning_task == "LINK_PREDICTION":
        ...
        if dataset_config.num_test is not MISSING and dataset_config.num_test != -1:
            ...
            assert os.path.getsize(test_edges_filepath) == dataset_config.num_test * num_cols * edges_dtype_size, \
                "Expected size for {} is {}, got {}".format(str(test_edges_filepath), \
            ...
            if not output_config.storage.full_graph_evaluation and not test_edges_partitions_filepath.exists() and \
                    output_config.storage.embeddings.type == "PARTITION_BUFFER":
                raise ValueError("{} does not exist, required for partitioned eval".format(test_edges_partitions_filepath))    
        ...
        if dataset_config.num_valid is not MISSING and dataset_config.num_valid != -1:
            ...
            assert os.path.getsize(validation_edges_filepath) == dataset_config.num_valid * num_cols * edges_dtype_size, \
                "Expected size for {} is {}, got {}".format(str(validation_edges_filepath), \
            ...
            if not output_config.storage.full_graph_evaluation and not valid_edges_partitions_filepath.exists() and \
                    output_config.storage.embeddings.type == "PARTITION_BUFFER":
                raise ValueError("{} does not exist, required for partitioned eval".format(valid_edges_partitions_filepath))
        ...
        if dataset_config.num_relations > 1:
            ...
            if num_lines != dataset_config.num_relations:
                raise ValueError("Expected {} lines in file {}, but found {}".format(dataset_config.num_relations, \
    ...
    if output_config.model.learning_task == "NODE_CLASSIFICATION":
        ...
        assert os.path.getsize(train_nodes_filepath) == dataset_config.num_train * nodes_dtype_size, \
                "Expected size for {} is {}, got {}".format(str(train_nodes_filepath), \
        ...
        if dataset_config.num_test is not MISSING and dataset_config.num_test != -1:
            ...
            assert os.path.getsize(test_nodes_filepath) == dataset_config.num_test * nodes_dtype_size, \
                "Expected size for {} is {}, got {}".format(str(test_nodes_filepath), \
        ...
        if dataset_config.num_valid is not MISSING and dataset_config.num_valid != -1:
            ...
            assert os.path.getsize(valid_nodes_filepath) == dataset_config.num_valid * nodes_dtype_size, \
                "Expected size for {} is {}, got {}".format(str(valid_nodes_filepath), \
...
def validate_storage_config(output_config):
    ...
    assert num_lines == num_partitions ** 2, ("Expected to see {} lines in {}, but found {} lines\n"
            "marius_preprocess was likely run with sqrt({}) partitions, "
            "but config file has {} partitions".format(num_partitions**2,
                                                        str(train_edges_partitions_filepath),
                                                        num_lines,
                                                        num_lines,
                                                        num_partitions))
    ...
    if test_edges_partitions_filepath.exists():
        ...
        assert num_lines == num_partitions ** 2, ("Expected to see {} lines in {}, but found {} lines\n"
                "marius_preprocess was likely run with sqrt({}) partitions, "
                "but config file has {} partitions".format(num_partitions**2,
                                                            str(test_edges_partitions_filepath),
                                                            num_lines,
                                                            num_lines,
                                                            num_partitions))
    ...
    if valid_edges_partitions_filepath.exists():
        ...
        assert num_lines == num_partitions ** 2, ("Expected to see {} lines in {}, but found {} lines\n"
                "marius_preprocess was likely run with sqrt({}) partitions, "
                "but config file has {} partitions".format(num_partitions**2,
                                                            str(valid_edges_partitions_filepath), 
                                                            num_lines,
                                                            num_lines,
                                                            num_partitions))
...
def check_encoder_layer_dimensions(output_config):
    ...
    for stage_idx, layer_list in enumerate(layers):
        for layer_idx, layer in enumerate(layer_list):
            if layer.type == "EMBEDDING":
                assert layer.input_dim == -1, "Expected Embedding layer to have no input, but found input dim as {}".format(layer.input_dim)
                assert layer.output_dim > 0, "Expected output dimension for Embedding layer to be > 0, but found {}".format(layer.output_dim)
                embeddings_output_dim = layer.output_dim if embeddings_output_dim == -1 else embeddings_output_dim
                assert embeddings_output_dim == layer.output_dim, \
                        "All Embedding Layers must have the same output dimension, found {} and {}".format(embeddings_output_dim, \
                                                                                                            layer.output_dim)
                continue
            
            if layer.type == "FEATURE":
                assert layer.input_dim == -1, "Expected Feature layer to have no input, but found input dim as {}".format(layer.input_dim)
                assert layer.output_dim > 0, "Expected output dimension for Feature layer to be > 0, but found {}".format(layer.output_dim)
                features_output_dim = layer.output_dim if features_output_dim == -1 else features_output_dim
                assert features_output_dim == layer.output_dim, \
                        "All Feature Layers must have the same output dimension, found {} and {}".format(features_output_dim, \
                                                                                                            layer.output_dim)
                continue
            ...
            if layer.type == "GNN":
                ...
                assert len(layers[stage_idx - 1]) > layer_idx, "Corresponding previous Layer for GNN Layer in Stage {} not found".format(stage_idx)
                assert layers[stage_idx - 1][layer_idx].output_dim == layer.input_dim, \
                        "GNN Layer in Stage {} has input dimension of {}, but output dimension of previous layers is {}".format(stage_idx, \
                                                                                                                                layer.input_dim,
                                                                                                                                layers[stage_idx - 1][layer_idx].output_dim)
                continue
    ...
    for i in range(1, len(layers)):
        ...
        if prev_stage_output_dim_sum != cur_stage_input_dim_sum:
            raise ValueError("Encoder layers dimension mismatch.\n"
                            "Output dimension of stage {} = {}\n"
                            "Input dimension of stage {} = {}".format(i-1, \
                                                                    prev_stage_output_dim_sum, \
                                                                    i, \
                                                                    cur_stage_input_dim_sum))
...
def check_gnn_layers_alignment(output_config):
    ...
    assert gnn_stage_count == len(neighbor_sampling_layers), \
            "#GNN Stages != #train_neighbor_sampling layers\nGNN Stages = {}, train_neighbor_sampling layers = {}".format(gnn_stage_count, \
                                                                                                                          len(neighbor_sampling_layers))
...
def get_storage_overheads(output_config):
    ...
    edge_mem_overhead = num_edges * 2 * edge_dtype_size * 2 if num_relations == 1 else num_edges * 3 * edge_dtype_size * 2
...
def check_full_graph_evaluation(output_config):
    ...
    if node_mem_overhead + rel_mem_overhead > mem_available:
        raise ValueError("full_graph_evaluation set to true, but not enough memory available for storing node and relation embeddings\n"
                        "Required memory = {} bytes, Available memory = {} bytes".format(str(node_mem_overhead + rel_mem_overhead), \
                                                                                        str(mem_available)))
