...
void init_trainer(py::module &m) {
    ...
    py::class_<SynchronousTrainer, Trainer, shared_ptr<SynchronousTrainer>>(m, "SynchronousTrainer")
        .def(py::init<shared_ptr<DataLoader> , shared_ptr<Model>, int>(),
             py::arg("dataloader"),
             py::arg("model"),
             py::arg("logs_per_epoch")=10);

    py::class_<PipelineTrainer, Trainer, shared_ptr<PipelineTrainer>>(m, "PipelineTrainer")
        .def(py::init<shared_ptr<DataLoader> , shared_ptr<Model>, shared_ptr<PipelineConfig>, int>(),
             py::arg("dataloader"),
             py::arg("model"),
