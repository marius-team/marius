...
void init_graph_storage(py::module &m) {
    ...
    py::class_<GraphModelStorage, std::shared_ptr<GraphModelStorage>>(m, "GraphModelStorage")
            ...
            .def(py::init<GraphModelStoragePtrs, shared_ptr<StorageConfig>>(),
                 py::arg("storage_ptrs"),
                 py::arg("storage_config"))
            .def(py::init([](shared_ptr<Storage> edges,
                             shared_ptr<Storage> nodes,
                             shared_ptr<Storage> node_features,
                             shared_ptr<Storage> node_embeddings,
                             shared_ptr<Storage> node_optimizer_state,
                     ...
                     if (train && node_optimizer_state == nullptr && node_embeddings != nullptr) {
                         ...
                         if (instance_of<Storage, InMemory>(node_embeddings)) {
                             ...
                             ptrs.node_optimizer_state = std::make_shared<PartitionBufferStorage>(optimizer_state_filename, std::dynamic_pointer_cast<PartitionBufferStorage>(node_embeddings)->options_);
                         } else {
                 ...
                 py::arg("nodes") = shared_ptr<Storage>(nullptr),
                 py::arg("node_features") = shared_ptr<Storage>(nullptr),
                 py::arg("node_embeddings") = shared_ptr<Storage>(nullptr),
                 py::arg("node_optim_state") = shared_ptr<Storage>(nullptr),
                 py::arg("node_labels") = shared_ptr<Storage>(nullptr),
                 py::arg("filter_edges") = std::vector<shared_ptr<Storage>>(),
                 py::arg("train") = false,
