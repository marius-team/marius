...
public:
    ...
    void reset_state() override {
        PYBIND11_OVERRIDE_PURE_NAME(void, Optimizer, "reset_state", reset_state);
    }

    void step() override {
        PYBIND11_OVERRIDE_PURE_NAME(void, Optimizer, "step", step);
    }
};
...
//        .def("load", &Optimizer::load, py::arg("input_archive"))
    ...
    py::class_<AdagradOptimizer, Optimizer, std::shared_ptr<AdagradOptimizer>>(m, "AdagradOptimizer")
            ...
            .def(py::init([](torch::OrderedDict<std::string, torch::Tensor> param_dict,
                             float learning_rate,
                             float eps,
                             float lr_decay,
                             float init_value,
            ...
            }), py::arg("param_dict"), py::arg("lr") = .1, py::arg("eps") = 1e-10, py::arg("lr_decay") = 0, py::arg("init_value") = 0, py::arg("weight_decay") = 0);
    ...
    py::class_<AdamOptimizer, Optimizer, std::shared_ptr<AdamOptimizer>>(m, "AdamOptimizer")
            ...
            }), py::arg("param_dict"), py::arg("lr") = .1, py::arg("eps") = 1e-8, py::arg("beta_1") = .9, py::arg("beta_2") = .999, py::arg("weight_decay") = 0, py::arg("amsgrad") = false);
