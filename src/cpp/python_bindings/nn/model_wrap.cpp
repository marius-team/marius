...
void init_model(py::module &m) {
    ...
    py::class_<Model, PyModel, torch::nn::Module, shared_ptr<Model>>(m, "Model", py::dynamic_attr())
        ...
        .def(py::init([](shared_ptr<GeneralEncoder> encoder, shared_ptr<Decoder> decoder, shared_ptr<LossFunction> loss, shared_ptr<Reporter> reporter, float sparse_lr) {
        ...
        .def("forward_nc", &Model::forward_nc, py::arg("node_embeddings"), py::arg("node_features"), py::arg("dense_graph"), py::arg("train"), py::call_guard<py::gil_scoped_release>())
        ...
        .def("zero_grad_all", &Model::zero_grad_all)
        .def("step", &Model::step)
    ...
    m.def("initModelFromConfig", [](pyobj python_config, pybind11::list devices_pylist, int num_relations, bool train) {
    ...
    m.def("load_from_file", [](string config_path, bool train) {
        ...
        model->load(config->storage->dataset->base_directory, train);
        return model;
