...
void init_relation_operators(py::module &m) {
    ...
    py::class_<HadamardOperator, RelationOperator, std::shared_ptr<HadamardOperator>>(m, "HadamardOperator")
        .def(py::init<>());

    py::class_<ComplexHadamardOperator, RelationOperator, std::shared_ptr<ComplexHadamardOperator>>(m, "ComplexHadamardOperator")
        .def(py::init<>());

    py::class_<TranslationOperator, RelationOperator, std::shared_ptr<TranslationOperator>>(m, "TranslationOperator")
        .def(py::init<>());

    py::class_<NoOp, RelationOperator, std::shared_ptr<NoOp>>(m, "NoOp")
        .def(py::init<>());
}
