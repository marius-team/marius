...
torch::Tensor compute_filter_corruption_cpu(shared_ptr<MariusGraph> graph, torch::Tensor edges, torch::Tensor corruption_nodes, bool inverse=false, bool global=false,
                                            LocalFilterMode local_filter_mode=LocalFilterMode::ALL,
                                            torch::Tensor deg_sample_indices=torch::Tensor());

torch::Tensor compute_filter_corruption_gpu(shared_ptr<MariusGraph> graph, torch::Tensor edges, torch::Tensor corruption_nodes, bool inverse=false, bool global=false,
                                            LocalFilterMode local_filter_mode=LocalFilterMode::ALL,
                                            torch::Tensor deg_sample_indices=torch::Tensor());

torch::Tensor compute_filter_corruption(shared_ptr<MariusGraph> graph, torch::Tensor edges, torch::Tensor corruption_nodes, bool inverse=false, bool global=false,
                                        LocalFilterMode local_filter_mode=LocalFilterMode::ALL,
                                        torch::Tensor deg_sample_indices=torch::Tensor());
...
public:
     ...
     */
    virtual std::tuple<torch::Tensor, torch::Tensor> getNegatives(shared_ptr<MariusGraph> graph, torch::Tensor edges=torch::Tensor(), bool inverse=false) = 0;
};
...
public:
    ...
    CorruptNodeNegativeSampler(int num_chunks,
                               int num_negatives,
...
public:
    ...
    CorruptRelNegativeSampler(int num_chunks,
                              int num_negatives,
                              bool filtered=false);
...
public:
    ...
    NegativeEdgeSampler(int num_chunks,
                        int num_negatives,
                        bool filtered=false);
