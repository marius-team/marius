...
def convert_to_tsv(output_dir=None):
    ...
    if temp != None:
        ...
        np.savetxt((Path("./training_data/edge_lhs_embedding.tsv")), lhs_rel_embs, fmt = "%f", delimiter='\t', newline='\n')
        np.savetxt((Path("./training_data/edge_rhs_embedding.tsv")), rhs_rel_embs, fmt = "%f", delimiter='\t', newline='\n')
