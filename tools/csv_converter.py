...
def split_dataset(input_dataset, validation_fraction, test_fraction, 
    ...
    if Path(train_file).exists(): Path(train_file).unlink()
    if Path(valid_file).exists(): Path(valid_file).unlink()
    if Path(test_file).exists(): Path(test_file).unlink()
    files = []

    num_line_skip = num_line_skip if num_line_skip != None else get_header_length(input_dataset, entry_regex)
    chunksize = 10 ** 7
...
def get_header_length(input_file, entry_regex):
    ...
    with open(input_file, 'r') as f:
        ...
        try:
            ...
            raise RuntimeError("Please give number of rows to skip at file header.")

        if a == n:
            raise RuntimeWarning("No nodes detected, dataset format may be incorrect.")
    ...
    for file in files:
        ...
        try:
            for chunk in pd.read_csv(file, sep=delim, header=None, chunksize=chunksize, skiprows=num_line_skip, usecols=data_cols, dtype=str):
                ...
                nodes = nodes | un_set
                if rel_idx != -1:
                    un_set = set(np.unique(chunk[chunk.columns.values[rel_idx]]))
                    rels = rels | un_set
        ...
        with open(train_out, "wb") as f:
            for file in temp_files:
                for chunk in pd.read_csv(file, sep=delim, header=None, chunksize=chunksize, skiprows=num_line_skip, usecols=data_cols, dtype=str):
                    ...
                    if rel_idx != -1:
                        rels = np.vectorize(rels_dict.get)(np.asarray(chunk[chunk.columns.values[rel_idx]]))
            ...
            if num_partitions > 1:
                ...
                edges, offsets = partition_edges(edges, num_partitions, len(node_ids))
                f.write(bytes(edges))
