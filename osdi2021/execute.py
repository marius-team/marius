...
def collect_metrics(info_log_only=False, dglke=False, pbg=False, eval_in_marius=False, experiment_name="marius"):
    if dglke:
        info_log = p.parse_dglke("tmp.txt")
    elif pbg:
        info_log = p.parse_pbg("tmp.txt")
        if eval_in_marius:
            info_log2 = p.parse_info_log("logs/%s_info.log" % experiment_name)
            info_log["MRR"] = info_log2["MRR"]
            info_log["Hits@1"] = info_log2["Hits@1"]
            info_log["Hits@5"] = info_log2["Hits@5"]
            info_log["Hits@10"] = info_log2["Hits@10"]
    else:
        info_log = p.parse_info_log("logs/%s_info.log" % experiment_name)
