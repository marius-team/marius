    ...
    def process(self):
        ...
        if self.only_train:
            ...
            if self.add_reverse_edges:
                ...
                test_mask[dataset_stats.num_train + dataset_stats.num_valid:dataset_stats.num_train + dataset_stats.num_valid + dataset_stats.num_test] = True
            else:
...
class MariusNCDataset(object):
    ...
    def process(self):
        ...
        # DGL was throwing cuda errors with things as int32
        edges_src = edges_src.to(torch.int64)
        edges_dst = edges_dst.to(torch.int64)
        self.graph = dgl.graph((edges_src, edges_dst), num_nodes=dataset_stats.num_nodes)
        ...
        if not self.only_sample:
            features = torch.from_numpy(np.fromfile(base_directory + "/nodes/features.bin",
                                                    dtype=np.float32)).view(-1, dataset_stats.node_feature_dim)
