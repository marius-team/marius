...
def run_ogbn_arxiv_disk_gpu(dataset_dir, results_dir, overwrite, enable_dstat, enable_nvidia_smi, show_output, short, num_runs=1):
    """
    Models: GraphSage
    ...
    marius = BASE_PATH / Path("marius_gs_disk_gpu.yaml")
    ...
    for i in range(num_runs):
        # Run Marius
        e.run_config(marius, results_dir / Path("ogbn_arxiv/marius_gs_disk_gpu"),
                     overwrite, enable_dstat, enable_nvidia_smi, show_output, i, "marius")


    reporting.print_results_summary([results_dir / Path("ogbn_arxiv/marius_gs_disk_gpu")])
