...
def run_fb15k237_mem_cpu(dataset_dir, results_dir, overwrite, enable_dstat, enable_nvidia_smi, show_output, short, num_runs=1):
    ...
    for i in range(num_runs):
        # Run Marius
        e.run_config(marius_gs_config, results_dir / Path("fb15k237/marius_gs_mem_cpu"), overwrite, enable_dstat,
                     enable_nvidia_smi, show_output, i, "marius")

        # Run DGL
        e.run_config(dgl_gs_config, results_dir / Path("fb15k237/dgl_gs_mem_cpu"), overwrite, enable_dstat, enable_nvidia_smi,
                     show_output, i, "dgl")

        # Run PyG
        e.run_config(pyg_gs_config, results_dir / Path("fb15k237/pyg_gs_mem_cpu"), overwrite, enable_dstat, enable_nvidia_smi,
                     show_output, i, "pyg")

    reporting.print_results_summary([results_dir / Path("fb15k237/marius_gs_mem_cpu"),
                                     results_dir / Path("fb15k237/dgl_gs_mem_cpu"),
                                     results_dir / Path("fb15k237/pyg_gs_mem_cpu")])
