...
BASE_PATH = Path("experiment_manager/system_comparisons/configs/freebase86m_disk_acc")


def run_freebase86m_gs_disk_acc(dataset_dir, results_dir, overwrite, enable_dstat, enable_nvidia_smi, show_output, short, num_runs=1):
    """
    Models: GraphSage
    Systems: Marius
    """

    dataset_name = "freebase86m_disk_acc"
    ...
    for i in range(num_runs):
        # Run Marius
        e.run_config(marius_gs, results_dir / Path("freebase86m_disk_acc/marius_gs"),
                     overwrite, enable_dstat, enable_nvidia_smi, show_output, i, "marius", omp_num_threads=4)

    reporting.print_results_summary([results_dir / Path("freebase86m_disk_acc/marius_gs")])