...
BASE_PATH = Path("experiment_manager/system_comparisons/configs/ogbn_papers100m_seq_time")


def run_ogbn_papers100m_seq_time(dataset_dir, results_dir, overwrite, enable_dstat, enable_nvidia_smi, show_output, short, num_runs=1):
    """
    Models: GraphSage
    Systems: Marius
    """

    dataset_name = "ogbn_papers100m_seq_time"
    ...
    for i in range(num_runs):
        # Run Marius
        e.run_config(marius_gs, results_dir / Path("ogbn_papers100m_seq_time/marius_gs"),
                     overwrite, enable_dstat, enable_nvidia_smi, show_output, i, "marius")

    reporting.print_results_summary([results_dir / Path("ogbn_papers100m_seq_time/marius_gs")])
